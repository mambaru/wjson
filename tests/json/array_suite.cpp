...
UNIT(array1, "Одномерный массив")
{
  ...
  t << is_true<expect>(e) << FAS_TESTING_FILE_LINE;
  t << equal<expect>(error, "Expected of '[': >>>1,2,3,4,5]") << FAS_TESTING_FILE_LINE;
  
  json = "[1,2,3,4,5";
  e.reset();
  ser(nums, json.begin(), json.end(), &e);
  error = e.message( json.begin(), json.end() );
  t << message("Test JSON error: ") << error;
  t << is_true<expect>(e) << FAS_TESTING_FILE_LINE;
  t << equal<expect>(error, "Unexpected end of ragment: [1,2,3,4,5<<<") << FAS_TESTING_FILE_LINE;
  ...
  t << message("Test JSON error: ") << error;
  t << is_true<expect>(e) << FAS_TESTING_FILE_LINE;
  t << equal<expect>(error, "Invalid json number: [1,2,3,>>>[4,5]") << FAS_TESTING_FILE_LINE;
...
UNIT(array2, "Двумерный массив")
{
  ...
  ser(nums, std::back_inserter(json) );
  t << equal<expect>( json, "[[1,2],[3,4]]") << FAS_TESTING_FILE_LINE;
  ...
  json.clear();
  ser(nums, std::back_inserter(json) );
  t << equal<expect>( json, "[[4,3],[2,1]]") << FAS_TESTING_FILE_LINE;
}
...
UNIT(array3, "Трехмерный массив")
{
  ...
  json.clear();
  ser(nums, std::back_inserter(json) );
  t << equal<expect>( json, "[[[4],[3]],[[2],[5]]]") << FAS_TESTING_FILE_LINE;
  ...
  json.clear();
  ser(nums, std::back_inserter(json) );
  t << equal<expect>( json, "[[[0],[0]],[[0],[5]]]") << FAS_TESTING_FILE_LINE;
  ...
  json.clear();
  ser(nums, std::back_inserter(json) );
  t << equal<expect>( json, "[[[0],[0]],[[0],[0]]]") << FAS_TESTING_FILE_LINE;
}
...
UNIT(array4, "std::vector< std::string >")
{
  ...
  t << equal<expect>( json, "[\"Привет\",\"Мир\",\"!\",\"世界你好!\"]" ) << FAS_TESTING_FILE_LINE;
  json = "[\"Привет\",\"Мир\",\"!\",\"\u4e16\u754c\u4f60\u597d!\"]";
  t << message( "re-serialization" ) << json << " -> " << "[\"Привет\",\"Мир\",\"!\",\"世界你好!\"]" ;
  json_error e;
  vs.clear();
  ser(vs, json.begin(), json.end(), &e);
  t << equal<expect>( vs[0], "Привет" ) << FAS_TESTING_FILE_LINE;
  t << equal<expect>( vs[1], "Мир" ) << FAS_TESTING_FILE_LINE;
  t << equal<expect>( vs[2], "!" ) << FAS_TESTING_FILE_LINE;
  t << equal<expect>( vs[3], "世界你好!" ) << FAS_TESTING_FILE_LINE;
}
...
UNIT(array5, "std::deque< int >")
{
  ...
  t << is_false<expect>(e) << FAS_TESTING_FILE_LINE;
  t << equal<expect>(dq, dq2) << FAS_TESTING_FILE_LINE;
}
...
UNIT(array8, "std::multiset< int >")
{
  ...
  ser(dq, std::back_inserter(json));
  t << equal<expect>(json, "[1,1,1,2,3,4,5]") << FAS_TESTING_FILE_LINE;
